        ...
        <header className="px-4 py-8 text-center space-y-4">
          ...
          <div className="flex gap-4 justify-center">
            <SocialDialog
              ...
              dialogDescription={(count) => `${count} people follows ${journal.name} by ${journal.user.name}`}
              dialogTrigger={(count) => <span
                className="text-neutral-600 font-medium hover:underline cursor-pointer"
              >
                {count} Followers
              </span>}
            ...
            />
            <p>
              <span className="font-medium">{journal.entries.length}</span> Entries
            </p>
          </div>
          <div className="flex justify-center gap-2">
            {auth.user && auth.user.id === journal.user.id ? (
              ...
              <FollowButton defaultValue={social.is_following}
                            followRoute={route('social.follow_journal', {
                              journal: journal.slug
                            })}
                            unfollowRoute={route('social.unfollow_journal', {journal: journal.slug})}
              />
            )}
          </div>
        </header>
